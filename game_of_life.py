...
class Grid:
    ...
    def __init__(self, w, h, p):
        ...
        m, n = self.grid.shape
        self.grid_padded = dpnp.empty((m + 2, n + 2), dtype=self.grid.dtype)
        self.grid_padded[1:-1, 1:-1] = self.grid  # copy input grid into the center of padded one
        self.grid_padded[0, 1:-1] = self.grid[-1]  # top row of padded grid
        self.grid_padded[-1, 1:-1] = self.grid[0]  # bottom
        self.grid_padded[1:-1, 0] = self.grid[:, -1]
        self.grid_padded[1:-1, -1] = self.grid[:, 0]
        self.grid_padded[0, 0] = self.grid[-1, -1]
        self.grid_padded[-1, -1] = self.grid[0, 0]
        self.grid_padded[0, -1] = self.grid[-1, 0]
        self.grid_padded[-1, 0] = self.grid[0, -1]
    ...
    def update(self):
        self.grid = grid_update(self.grid, self.grid_padded, self.rules)
